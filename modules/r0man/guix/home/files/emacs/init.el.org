...
#+begin_src emacs-lisp
  (use-package projectile
    ...
    ;; `alien' indexing shells out to git ls-files, which is a whole
    ;; process spawn over TRAMP -- and C-x C-f is projectile-find-file,
    ;; so a remote project pays it on every invocation.  Caching makes it
    ;; a one-off.  Watches keep local projects fresh; remote projects are
    ;; deliberately never watched (one watch per directory means one
    ;; round-trip each), so refresh those with C-c p i.
