...
#+begin_src emacs-lisp
  (use-package modus-themes
    :commands (modus-themes-load-random
               modus-themes-rotate))
#+end_src
...
Use the tree-sitter based Markdown mode from Emacs 31 instead of
...
Configure mu with:
  ...
  mu init --maildir=~/Mail --my-address=roman@burningswell.com --my-address=roman.scherer@burningswell.com
  mu index
...
#+begin_src emacs-lisp
  (use-package tramp
    ...
    ;; Direct async processes give magit no pipe to talk over unless it
    ;; requests a pty; without this magit hangs or garbles output on
    ;; remote repositories.
...
The Guix =tree-sitter-markdown= package installs the inline grammar
as =libtree-sitter-markdown_inline.so=, but Emacs looks for
...
#+begin_src emacs-lisp
  (use-package treesit
    ...
    (add-to-list 'treesit-load-name-override-list
                 '(markdown-inline "libtree-sitter-markdown_inline"
...
Project management with the built-in project.el.  Unlike Projectile it
does not hook into find-file, dired or window-configuration hooks, so
it never probes for project roots over TRAMP (or anywhere else) unless
a project command is explicitly invoked.

#+begin_src emacs-lisp
  (use-package project
    :preface
    (defun r0man/project-prefix-command ()
      "Show `project-prefix-map' with which-key and run the chosen command.
Behaves like typing C-x p in the current project, which
makes it useful as an entry in `project-switch-commands'."
      (interactive)
      (require 'which-key)
      (which-key-show-keymap 'project-prefix-map t)
      (unwind-protect
    ...
    (project-switch-commands
     '((project-find-file "Find file" "f")
       ...
       (ghostel-project "Terminal" "m")
       (claude-code-ide "Claude" "c")
       (beads "Beads" "B")
       (gascity "Gas City" "G")
       (r0man/project-prefix-command "C-x p" "o")))
...
Edit grep and ripgrep result buffers (e.g. from `embark-export' or
=M-x grep=) and apply the changes to the underlying files with =C-c
C-c=.  Enter the editable mode with =C-c C-p=.
