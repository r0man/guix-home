...
#+begin_src emacs-lisp
  ...
  (defun r0man/nubank-precision-p ()
    "Non-nil when running as user `roman' on host `precision'."
    (and (string= (user-login-name) "roman.scherer")
...
#+begin_src emacs-lisp
  ...
  (defun current-font-size ()
    ...
    (cond ((equal "m1" (system-name))
          ...
          ((equal "precision" (system-name))
           (if (getenv "WAYLAND_DISPLAY") 17 27))
          (t 19)))
...
#+begin_src emacs-lisp
  (use-package ellama
    ...
    (let* ((claude-name (caar r0man/claude-models))
           (default-name (if (r0man/nubank-precision-p)
                             (format "Nubank %s" claude-name)
                           claude-name)))
    ...
    (defun guix-home-reconfigure ()
      ...
      (with-current-buffer (get-buffer-create "*Guix Home Reconfigure*")
        ...
        (let ((default-directory "~/workspace/guix-home"))
          (async-shell-command "~/.config/guix/current/bin/guix home reconfigure -L modules modules/r0man/guix/home/environments/precision.scm" (current-buffer)))))
#+end_src
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (let* ((claude-models (mapcar (lambda (m) (intern (cdr m)))
      ...
      (if (r0man/nubank-precision-p)
          (setq-default gptel-backend nubank-backend
                        gptel-model   (intern (format "anthropic/%s"
                                                      (cdar r0man/claude-models))))
...
#+begin_src emacs-lisp
  (use-package tramp
    ...
    ;; Use direct SCP for remote copying (faster than default method)
