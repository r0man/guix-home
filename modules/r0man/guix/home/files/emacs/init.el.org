...
My Emacs init.el in [[http://orgmode.org/][Org Mode]]'s [[http://orgmode.org/worg/org-contrib/babel][Babel]] format.
...
** LLM models and helpers

Shared model lists, endpoints, and credential helpers used by the
=ellama= and =gptel= sections below.

#+begin_src emacs-lisp
  (defvar r0man/openai-models
    '(("OpenAI GPT-5.5"      . "gpt-5.5")
      ("OpenAI GPT-5.4"      . "gpt-5.4")
      ("OpenAI GPT-5.4 mini" . "gpt-5.4-mini"))
    "OpenAI chat models as (DISPLAY-NAME . MODEL-ID); CAR is the default.")

  (defvar r0man/claude-models
    '(("Claude Opus 4.7"   . "claude-opus-4-7")
      ("Claude Sonnet 4.6" . "claude-sonnet-4-6")
      ("Claude Haiku 4.5"  . "claude-haiku-4-5-20251001"))
    "Claude chat models as (DISPLAY-NAME . MODEL-ID); CAR is the default.")

  (defvar r0man/gemini-models
    '(("Gemini 2.5 Pro"   . "gemini-2.5-pro")
      ("Gemini 2.5 Flash" . "gemini-2.5-flash"))
    "Gemini chat models as (DISPLAY-NAME . MODEL-ID); CAR is the default.")

  (defconst r0man/nubank-litellm-host
    "ist-prod-litellm.nullmplatform.com"
    "Nubank LiteLLM proxy host.")

  (defconst r0man/nubank-litellm-url
    (concat "https://" r0man/nubank-litellm-host "/")
    "Nubank LiteLLM proxy URL.")

  (defun r0man/anthropic-key ()
    "Return the personal Anthropic API key from auth-source."
    (auth-source-pick-first-password
     :host "api.anthropic.com" :user "personal"))

  (defun r0man/openai-key ()
    "Return the personal OpenAI API key from auth-source."
    (auth-source-pick-first-password
     :host "openai.com" :user "ellama"))

  (defun r0man/nubank-litellm-key ()
    "Return the Nubank LiteLLM proxy API key from auth-source."
    (auth-source-pick-first-password
     :host r0man/nubank-litellm-host :user "apikey"))

  (defun r0man/nubank-llm-provider (prefix display-name model-id key
                                           &optional embedding-model)
    "Cons cell suitable for `ellama-providers'.
PREFIX is the LiteLLM route prefix (e.g. \"anthropic\", \"openai\",
\"gemini\"). DISPLAY-NAME is rendered as \"Nubank DISPLAY-NAME\".
MODEL-ID is the upstream chat model. KEY is the LiteLLM API key.
EMBEDDING-MODEL, if non-nil, sets the embedding model."
    (cons (format "Nubank %s" display-name)
          (apply #'make-llm-openai-compatible
                 :url r0man/nubank-litellm-url
                 :key key
                 :chat-model (format "%s/%s" prefix model-id)
                 (when embedding-model
                   (list :embedding-model embedding-model)))))
#+end_src
...
** Fonts
#+begin_src emacs-lisp
  (defconst default-font-family "JetBrains Mono"
    "Default font family used by `current-font' to build the spec.")

  (defun current-font-size ()
    "Get the font pixel size for the current system."
    (cond ((equal "m1" (system-name))
           (if (getenv "WAYLAND_DISPLAY") 19 17))
          (t 19)))

  (defun current-font ()
    "Return the font spec to use on this machine."
    (format "%s:pixelsize=%d:weight=ultralight"
            default-font-family (current-font-size)))

  (defun set-default-font ()
    "Set the default frame font to `current-font'."
    (interactive)
...
#+begin_src emacs-lisp
  (use-package gastown
    :disabled
    :bind (("C-x C-g g" . gastown))
    :commands (gastown)
    :load-path ("~/workspace/gastown.el/lisp"
                "~/workspace/gastown.el/lisp/test")
    :bind ("C-c g" . gastown))
#+end_src
** Gascity
#+begin_src emacs-lisp
  (use-package gascity
    :bind (("C-x C-g g" . gascity))
    :commands (gascity)
    :load-path ("~/workspace/gascity.el/lisp"
                "~/workspace/gascity.el/lisp/test")
    :bind ("C-c g" . gascity))
#+end_src
** Bug Reference
...
#+begin_src emacs-lisp
  (use-package claude-code-ide
    :load-path ("~/workspace/claude-code-ide.el")
    :commands (claude-code-ide)
    :bind ("C-c C-'" . claude-code-ide-menu)
    :custom
    ;; (claude-code-ide-cli-extra-flags "")
    ;; (claude-code-ide-cli-path "container-claude")
...
#+begin_src emacs-lisp
  (use-package eca
    ...
    (eca-custom-command '("~/.local/bin/eca-server-authinfo" "--log-level" "debug" "--verbose")))
#+end_src
** Efrit
#+begin_src emacs-lisp
  (use-package efrit
    :commands (efrit-chat
               ...
               efrit-unified-do)
    :custom
    (efrit-api-key (lambda () (auth-source-pick-first-password :host "api.anthropic.com" :user "personal"))))
#+end_src
** Eldoc
...
#+begin_src emacs-lisp
  (use-package ellama
    ...
    (require 'llm-claude)
    (require 'llm-openai)
    (let ((anthropic-key (r0man/anthropic-key))
          (openai-key    (r0man/openai-key))
          (nullm-key     (r0man/nubank-litellm-key))
          providers)
      (dolist (model r0man/claude-models)
        (let ((name (car model)) (id (cdr model)))
          (push (cons name (make-llm-claude :key anthropic-key :chat-model id))
                providers)
          (push (r0man/nubank-llm-provider "anthropic" name id nullm-key)
                providers)))
      (dolist (model r0man/openai-models)
        (let ((name (car model)) (id (cdr model)))
          (push (cons name (make-llm-openai :key openai-key :chat-model id))
                providers)
          (push (r0man/nubank-llm-provider "openai" name id nullm-key
                                           "text-embedding-3-small")
                providers)))
      (dolist (model r0man/gemini-models)
        (let ((name (car model)) (id (cdr model)))
          (push (r0man/nubank-llm-provider "gemini" name id nullm-key)
                providers)))
      (setq ellama-providers (append (nreverse providers) ellama-providers)))
    (let* ((claude-name (caar r0man/claude-models))
           (default-name claude-name))
      (setq ellama-provider
            (alist-get default-name ellama-providers nil nil #'string=))))
    ...
    (defun guix-home-reconfigure ()
      ...
      (with-current-buffer (get-buffer-create "*Guix Home Reconfigure*")
        ...
        (let ((default-directory "~/workspace/guix-home"))
          (async-shell-command "~/.config/guix/current/bin/guix home reconfigure -L modules modules/r0man/guix/home/environments/m1.scm" (current-buffer)))))
#+end_src
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (let* ((claude-models (mapcar (lambda (m) (intern (cdr m)))
                                  r0man/claude-models))
           (openai-models (mapcar (lambda (m) (intern (cdr m)))
                                  r0man/openai-models))
           (nubank-models
            (append
             (mapcar (lambda (m) (intern (format "openai/%s" (cdr m))))
                     r0man/openai-models)
             (mapcar (lambda (m) (intern (format "anthropic/%s" (cdr m))))
                     r0man/claude-models)
             (mapcar (lambda (m) (intern (format "gemini/%s" (cdr m))))
                     r0man/gemini-models)))
           (claude-backend
            (gptel-make-anthropic "Claude"
              :stream t
              :models claude-models
              :key #'r0man/anthropic-key)))
      (gptel-make-openai "Nubank"
        :stream t
        :models nubank-models
        :host r0man/nubank-litellm-host
        :key #'r0man/nubank-litellm-key)
      (gptel-make-openai "OpenAI"
        :stream t
        :models openai-models
        :key #'r0man/openai-key)
      (setq-default gptel-backend claude-backend
                    gptel-model   (intern (cdar r0man/claude-models))))
    :custom
    (when (file-exists-p "~/.guix-profile/bin/curl")
...
Full color support in terminals.

#+begin_src emacs-lisp
  (add-to-list 'term-file-aliases '("screen.xterm-256color" . "xterm-256color"))
  (add-to-list 'term-file-aliases '("xterm-kitty" . "xterm-direct"))
...
#+begin_src emacs-lisp
  (use-package tramp
    :defer t
    :config
    ;; Performance optimizations from "Making TRAMP go Brrrr"
    ;; https://coredumped.dev/2025/06/18/making-tramp-go-brrrr

    ;; Reduce verbose logging (10 is too high for normal use)
    ;; Set to 10 only when actively debugging TRAMP issues
    (setq tramp-verbose 1)

    ;; Inhibit remote file locks (reduces remote filesystem operations)
    (setq remote-file-name-inhibit-locks t)

    ;; Disable auto-save for remote files (prevents extra writes)
    (setq remote-file-name-inhibit-auto-save-visited t)

    ;; The default of 10 seconds throws cached file attributes away
    ;; constantly, so dired re-stats the whole listing seconds after
    ;; drawing it.  At ~140ms round-trip to burningswell.com that
    ;; dominates everything else.  Ten minutes is safe as long as nothing
    ;; writes the tree behind our back; `g' in dired still forces a
    ;; refresh when it does.
    (setq remote-file-name-inhibit-cache 600)

    ;; Only governs copies between two *remote* hosts.  It does nothing
    ;; for the usual remote->local direction.
    (setq tramp-use-scp-direct-remote-copying t)

    ;; Keep TRAMP from appending its own ControlMaster options: it uses
    ;; ControlPersist=no, so its master dies immediately and every ssh it
    ;; spawns pays the full handshake (~2s to a high-latency host).  nil
    ;; makes TRAMP inherit the persistent master from ~/.ssh/config, see
    ;; modules/r0man/guix/home/ssh.scm.
...
#+begin_src emacs-lisp
  (use-package projectile
    ...
    ;; Projectile hooks into dired-before-readin-hook, find-file-hook and
    ;; window-configuration-change-hook, and each one probes for a project
    ;; root: 61 marker files in `projectile-project-root-files' alone,
    ;; stat'd at every level while walking up to /.  Locally that is free.
    ;; Over TRAMP it is ~137 serialised round-trips -- measured at 79s to
    ;; open one 19-entry dired on gascity@burningswell.com, and it repeats
    ;; for every directory entered because nothing up that tree is a
    ;; project, so there is not even a hit to cache.  Skip the three hooks
    ;; on remote directories; local projects keep every feature.
    (defun projectile-skip-when-remote (fn &rest args)
      "Apply FN to ARGS only when `default-directory' is local."
      (unless (file-remote-p default-directory)
        (apply fn args)))

    (advice-add 'projectile-track-known-projects-find-file-hook
                :around #'projectile-skip-when-remote)
    (advice-add 'projectile-update-mode-line-on-window-change
                :around #'projectile-skip-when-remote)
    (advice-add 'projectile-find-file-hook-function
                :around #'projectile-skip-when-remote)
...
** Sooper Whisper
#+begin_src emacs-lisp
  (use-package sooper-whisper
    :commands
    (sooper-whisper
     sooper-whisper-record
     sooper-whisper-stop
     sooper-whisper-cancel
     sooper-whisper-push-to-talk
     sooper-whisper-menu
     sooper-whisper-doctor)
    :load-path
    ("~/workspace/sooper-whisper.el/src"
     "~/workspace/sooper-whisper.el/tests")
    :custom
    (sooper-whisper-gpu-device-index 1)
    (sooper-whisper-debug 'verbose))
#+end_src
...
Emacs terminal emulator powered by libghostty-vt with built-in
multi-buffer support.

#+begin_src emacs-lisp
  (use-package ghostel
    :bind (("C-x M" . ghostel-new)
           ("C-x m" . ghostel-next)
           :map ghostel-mode-map
           ("M-SPC" . ghostel-send-meta-space))
    :preface
    (defun ghostel-new ()
      "Always start a new Ghostel terminal buffer."
      (interactive)
      (ghostel '(4)))
    (defun ghostel-send-meta-space ()
      "Send raw ESC+SPC to the terminal so tmux's `M-Space' prefix fires.
  ghostel's native key encoder silently drops Meta+Space (verified by
  stracing the PTY master: `M-x' emits \"\\ex\" but `M-SPC' emits
  nothing), so routing through `ghostel--send-event' would send no
  bytes.  Write the legacy Alt encoding directly instead."
      (interactive)
...
Enable clipboard support in terminal emulators.
