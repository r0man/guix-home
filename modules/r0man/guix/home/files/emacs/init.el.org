  ...
  (defun monitor-connected-p (monitor)
    ...
    (if (executable-find "xrandr")
        (not (string= "" (shell-command-to-string
                          (format "xrandr --listmonitors | grep %s" monitor))))
      nil))
#+end_src
** Abbrev mode
...
#+begin_src emacs-lisp
  (use-package beads
    ...
    :commands (beads)
    :load-path ("~/workspace/beads.el/bde-koh7/lisp"
                "~/workspace/beads.el/bde-koh7/lisp/test")
    :bind ("C-c b" . beads)
    :custom
...
#+begin_src emacs-lisp
  (defun xresources ()
    "Reload the ~/.Xresources configuration."
    (interactive)
    (if (executable-find "xrdb")
        (progn
          (shell-command "xrdb -merge ~/.Xresources")
          (message "X resources reloaded."))
      (message "xrdb not found (not on X11).")))
#+end_src
...
#+begin_src emacs-lisp
  (defun guix-system-p ()
    "Return t if running on a Guix System."
    (interactive)
    (file-exists-p "/run/current-system/configuration.scm"))

  (defun guix-available-p ()
    "Return t if Guix is available on this system."
...
#+begin_src emacs-lisp
  (defun guix-home-reconfigure ()
    ...
    (interactive)
    (unless (guix-available-p)
      (user-error "Guix is not available on this system"))
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    :custom
    (gptel-use-curl (if (file-exists-p "/gnu/store")
                        "~/.guix-profile/bin/curl"
                      (or (executable-find "curl") "curl"))))
#+end_src
** History
...
#+begin_src emacs-lisp
  (use-package plz
    :defer t
    :config
    ;; When not on a Guix system, use curl from the appropriate location
    (unless (file-exists-p "/run/current-system")
      (setq plz-curl-program
            (if (file-exists-p "/gnu/store")
                "~/.guix-profile/bin/curl"
              (or (executable-find "curl") "curl")))))
#+end_src
