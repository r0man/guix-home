...
#+begin_src emacs-lisp
  (use-package clojure-mode
    ...
    (defun clojure-toggle-between-implementation-and-test ()
      "Toggle between a Clojure implementation file and its test file.
Assumes the conventional src/ and test/ layout with a _test suffix."
      (interactive)
      (let ((file (buffer-file-name)))
        (unless file
          (user-error "Buffer is not visiting a file"))
        (find-file
         (if (string-match-p "_test\\.clj[sc]?\\'" file)
             (replace-regexp-in-string
              "/test/" "/src/"
              (replace-regexp-in-string "_test\\(\\.clj[sc]?\\)\\'" "\\1" file))
           (replace-regexp-in-string
            "/src/" "/test/"
            (replace-regexp-in-string "\\(\\.clj[sc]?\\)\\'" "_test\\1" file))))))
    (define-key clojure-mode-map (kbd "C-c t") #'clojure-toggle-between-implementation-and-test)
...
The =C-c p= prefix is kept from the Projectile days, alongside the
default =C-x p=.  The =s g= / =s r= consult bindings shadow
`project-shell' on =s=, which vterm makes redundant anyway.

#+begin_src emacs-lisp
  (use-package project
    :init
    (defun project-find-file-dwim ()
    ...
    :bind
    (("C-x C-f" . project-find-file-dwim)
    ...
    :custom
    ;; Treat these as project roots in addition to version control roots,
    ;; e.g. for Dart/Flutter, Bazel and Guix projects.
    (project-vc-extra-root-markers '("pubspec.yaml" "BUILD" "guix.scm")))
#+end_src
** Ruby mode
