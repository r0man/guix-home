---
name: design
description: Structured design exploration — examines a design problem from several specialized dimensions (API/interface, data model, UX, scalability, security, integration) and synthesizes a unified design document with options, trade-offs, and a phased implementation plan. Use this skill whenever the user wants to design or architect something before building it: "design X", "how should I architect Y", "explore approaches for Z", "think through the design of", "write a design doc for", "what are the trade-offs of", or a redesign of an existing system. Prefer this over jumping straight to a single-perspective proposal whenever the problem is non-trivial — a one-angle design misses the constraints the other dimensions surface. This produces the design doc; use the shiny skill when the user wants to design *and then implement* a change end-to-end.
---
