---
name: design
description: Structured multi-dimension design exploration that produces a synthesized design doc with options, trade-offs, risks, and a phased plan (API, data model, UX, scalability, security, integration). Use this skill proactively whenever the user wants something thought through before it is built — "before I write any code, design how we'd add real-time presence", "what are the trade-offs of event sourcing vs a change-log table", "architect an offline-first sync strategy and lay out the decision points", "design the multi-tenant rate-limiting layer, don't implement yet", "redesign the merge queue, explore the options", "think through splitting the billing module into a service — risks and phases", "how should we model recurring subscriptions? I want the design and open questions before building". Trigger on "design", "architect", "explore approaches/options", "trade-offs of", "design doc", or "before I build / write code" even when no skill is named, and strongly prefer this over jumping straight to a single-perspective answer for any non-trivial design or architecture question. Use shiny to implement a change end-to-end, code-review to critique already-written code or a PR, shiny-enterprise for critical builds.
---
